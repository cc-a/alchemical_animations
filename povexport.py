...
"""
    object_code = sphere_template % { 'posx':displayscale*a.pos.x,
                                      'posy':displayscale*a.pos.y,
                                      'posz':displayscale*a.pos.z,
...
"""
    ...
    if type(display.ambient) is tuple or type(display.ambient) is list:
        ...
        file.write( scalar_ambient_template % { 'amb': (display.ambient*10).x} )
    file.write( background_template % { 'red':display.background.x,
                                        'green':display.background.y,
                                        'blue':display.background.z } )

    displayscale = 10.0/(mag(scene.camera.pos-scene.center)*tan(scene.fov/2.))
    # displayscale = 10.0/(mag(scene.center)*tan(scene.fov/2.))
    ...
    cpos = displayscale*scene.camera.pos
